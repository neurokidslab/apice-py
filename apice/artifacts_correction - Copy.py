    ...
    def _spherical_spline_interpolation(self, raw):
        ...
        for ep in np.arange(n_epochs):
            ...
            for segmentIndex in np.arange(np.shape(badIntervals_ep)[0]):
                allBadChannels_ep.append(chaInterpolate_ep[segmentIndex] | badChannelsPerEpoch)

            # transform to indexes across epochs
            badIntervals_ep = badIntervals_ep + ep * n_samples
            
            # concatenate
            badIntervals.append(badIntervals_ep)
            chaInterpolate.append(chaInterpolate_ep)
            allBadChannels.append(allBadChannels_ep)

        badIntervals = np.vstack(badIntervals)
        chaInterpolate = np.vstack(chaInterpolate)
        allBadChannels = np.vstack(allBadChannels)
        
        # Initialize matrices
        interpolatedData = raw._data.copy()
        if len(raw._data.shape) == 3:
            interpolatedData = np.transpose(interpolatedData, (1,2,0))
        ...
        for segmentIndex in np.arange(np.shape(badIntervals)[0]):
            ...
            if (np.sum(all_bad_channels) / n_channels) <= self.params['p']:
                ...
                initialSegmentIndices.append(segmentStart)
                finalSegmentIndices.append(segmentEnd)  
                ...
                for i in bad_ch:
                    interpolationMatrix[i, np.arange(segmentStart, segmentEnd)] = True

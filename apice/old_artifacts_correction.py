...
class SegmentSphericalSplineInterpolation(ArtCorrection):
    ...
    def _spherical_spline_interpolation(self, raw):
        ...
        if len(raw._data.shape) == 2:
            bct_to_interpolate = bct_to_interpolate[np.newaxis, :, :]
        ...
        #     if len(np.shape(raw._data))==2:
        #         bad_channels_ep = art._bc[:,0]
        #     else:
        #         bad_channels_ep = np.squeeze(art._bc[ep,:,:])
            
        #     # data to interpolate
        #     if len(np.shape(raw._data))==2:
        #         rej_int = bct_to_interpolate
        #     else:
        #         rej_int = np.squeeze(bct_to_interpolate[ep,:,:])
                
        #     # Identify the segments and channels to interpolate
        #     bad_if_ep, cha_interpolate_ep = find_segments_interpolation_spline(rej_int, bad_channels_ep)
        ...
        #     bad_if_ep = bad_if_ep + ep * n_samples
            
        #     # concatenate
        #     bad_if.append(bad_if_ep)
        #     cha_bad = cha_bad + cha_bad_ep
        #     cha_interpolate = cha_interpolate + cha_interpolate_ep
